...
ЧТО УМЕЕТ
• 26 сервисов Google в одном окне: почта, календарь, документы, хранилище файлов, видеовстречи и другие.
• «Избранное» и порядок плиток настраиваются перетаскиванием, ненужные плитки можно скрыть.
• Число непрочитанных писем на иконке расширения и на плитке почты.
...
Расширение читает служебный фид почты в сессии, уже открытой в браузере. OAuth, пароли и доступ к аккаунту не нужны: достаточно войти в почту в этом браузере. Из фида берутся только число непрочитанных писем и их даты.

Фид содержит не больше 20 последних писем. Если за выбранный период непрочитанных больше, счётчик покажет «20+».
...
Магазин выдал расширению свой ID — `ifdinmakpglpbkmnaaeaplbgphbkgdgj`. Он не совпадает с
ID сборки из GitHub Releases: это два разных расширения, у каждого свои настройки.

Как только расширение опубликуют, все кнопки и ссылки на магазин в README начнут
работать от одной ссылки `[webstore]` в самом конце файла — замените её на
`https://chromewebstore.google.com/detail/ifdinmakpglpbkmnaaeaplbgphbkgdgj`. После
публикации бейджи в шапке можно заменить на живые:
`https://img.shields.io/chrome-web-store/v/ifdinmakpglpbkmnaaeaplbgphbkgdgj`, а также
`/users/<ID>` и `/rating/<ID>`.

## Отказ по первой отправке, сентябрь 2026: Yellow Argon (keyword stuffing)

Первая отправка отклонена по пункту «Злоупотребление ключевыми словами». Претензия была
к одной строке описания: там подряд перечислялись десять продуктов Google («Почта, Диск,
Календарь, Документы, Таблицы, Gemini, YouTube, Карты, Meet, Переводчик»). Формулировка
проверяющих: `>5 entities`.

Описание выше уже исправлено: вместо списка названий — родовые слова («почта, календарь,
документы, хранилище файлов, видеовстречи»). Правило на будущее: **не перечисляйте
подряд больше пяти чужих продуктов** ни в описании, ни в названии, ни в тексте на
скриншотах и промо-баннере. Полный список сервисов уместен в README, но не в метаданных
магазина.

Отказ относится к метаданным, а не к коду, поэтому пересобирать архив не нужно. Что
делать: вставить новое описание на вкладке **Store listing**, сохранить черновик и снова
нажать **Submit for review**. Апелляция здесь не нужна — претензия справедливая, а
повторная отправка с исправленным текстом проходит быстрее.
